...
export default function Doing() {
  ...
  function handleSubmit(e) {
    ...
    axios.post('http://localhost:5000/doing', newTodo)
      .then(res => {
...
function handleUpdate(e){
  ...
  axios.put(`http://localhost:5000/doing/${todoUpdate}`, {text: todoEditing})
    .then(res => {
      console.log(res.data);
      setTodoUpdate(null)
      setTodoEditing(null)
  ...
  function deleteTodo(e, id) {
    ...
    axios.delete(`http://localhost:5000/doing/${id}`)
      .then(res => {
        console.log(res.data);
      })
      .catch(err => {
  ...
  function completeTodo(e, id) {
    ...
    axios.patch(`http://localhost:5000/doing/${id}`, { completed: true })
      .then(res => {
        console.log(res.data);
  ...
  useEffect(() => {
    const getTodos = async () => {
      await axios.get('http://localhost:5000/doing')
        .then(res => {
          setTodos(res.data)
        })
        .catch(err => {
  ...
  return (
    <div className="justify-between items-center mb-4">
      <div className='flex justify-between items-center bg-white rounded-md py-3 px-4 m-4 w-auto'>
        <h1 className="font-bold uppercase">Doing List</h1>
