...
export default function Done() {
  ...
  function handleSubmit(e) {
    ...
    axios.post('http://localhost:5000/done', newTodo)
      .then(res => {
...
function handleUpdate(e){
  ...
  axios.put(`http://localhost:5000/done/${todoUpdate}`, {text: todoEditing})
    .then(res => {
      console.log(res.data);
      setTodoUpdate(null)
      setTodoEditing(null)
  ...
  function deleteTodo(e, id) {
    ...
    axios.delete(`http://localhost:5000/done/${id}`)
      .then(res => {
        console.log(res.data);
      })
      .catch(err => {
  ...
  function completeTodo(e, id) {
    ...
    axios.patch(`http://localhost:5000/done/${id}`, { completed: true })
      .then(res => {
        console.log(res.data);
      })
      .catch(err => {
  ...
  useEffect(() => {
    const getTodos = async () => {
      await axios.get('http://localhost:5000/done')
        .then(res => {
          setTodos(res.data)
        })
        .catch(err => {
  ...
  return (
    <div className="justify-between items-center mb-4">
      <div className='flex justify-between items-center bg-white rounded-md py-3 px-4 m-4 w-auto'>
        <h1 className="font-bold uppercase">Done List</h1>
